...
namespace OmsiVisualInterfaceNet
{
    public class OmsiManager : IDisposable
    {
        ...
        public async Task Initialize()
        {
            ...
            tank1 = Convert.ToDouble(constantsManager.FindConstantValue("bremse_V_tank01"));
            tank2 = Convert.ToDouble(constantsManager.FindConstantValue("bremse_V_tank02"));
        ...
        {
            CurrentVehicle?.SetVariable("lights_sw_warnblinker", on ? 1 : 0);
            CurrentVehicle?.SetVariable("cp_taster_warnblinker", on ? 1 : 0);
        }
        public void SetActiaScreenUp(bool pressed)  
        { 
            CurrentVehicle?.SetTrigger("Actia_Dash_changemode_up", true);
            CurrentVehicle?.SetTrigger("Actia_Dash_changemode_up_off", true);
        }
        public void SetActiaScreenDown(bool pressed)  
        { 
            CurrentVehicle?.SetTrigger("Actia_Dash_changemode_down", true);
            CurrentVehicle?.SetTrigger("Actia_Dash_changemode_down_off", true);
        ...
        public void SetSuspensionUp(bool pressed) => CurrentVehicle?.SetTrigger($"cp_kneel_up_toggle{(pressed ? "" : "_off")}", true);
        public void SetSuspensionDown(bool pressed) => CurrentVehicle?.SetTrigger($"cp_kneel_down_toggle{(pressed? "" : "_off")}", true);
        ...
        public void SetDoorButton(int door, bool pressed)
        {
            ...
            if (door == 0)
            {
                CurrentVehicle?.SetTrigger( "bus_doorfront0",true);
                CurrentVehicle?.SetTrigger( "bus_doorfront0_off",true);
            }
            else if (door == 1)
            {
                CurrentVehicle?.SetTrigger("bus_doorfront1", true);
                CurrentVehicle?.SetTrigger("bus_doorfront1_off", true);
            }
            else if(door == 2)
            {
                CurrentVehicle?.SetTrigger("bus_doorfront2", true);
                CurrentVehicle?.SetTrigger("bus_doorfront2_off", true);
            }
        ...
        public void SetGear(int pos, bool pressed)
        {
            ...
            try
            {
                ...
                switch (pos)
                {
                    case -1:
                        if (pressed)
                            CurrentVehicle.SetTrigger("automatic_R",true);
                        else
                            CurrentVehicle.SetTrigger("automatic_R_off", true);
                        break;
                    case 0:
                        if (pressed)
                            CurrentVehicle.SetTrigger("automatic_N", true);
                        else
                            CurrentVehicle.SetTrigger("automatic_N_off", true);
                        break;
                    case 1:
                        if (pressed)
                            CurrentVehicle.SetTrigger("automatic_D", true);
                        else
                            CurrentVehicle.SetTrigger("automatic_D_off", true);
                        break;
                    default:
                        return;
                }
        ...
        public void SetFrontFog(bool on) => CurrentVehicle?.SetVariable("lights_nebel", on ? 1 : 0);
        public void SetBackFog(bool on) => CurrentVehicle?.SetVariable("lights_nebelschluss", on ? 1 : 0);
        ...
        public void SetHighBeam(bool on) 
        {
            ...
            {
                CurrentVehicle?.SetVariable("lights_sw_fern", 1);
                lastHighBeamState = true;
            }
            else
            {
                CurrentVehicle?.SetVariable("lights_sw_fern", 0);
