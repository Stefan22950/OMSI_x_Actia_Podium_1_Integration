...
namespace OmsiVisualInterfaceNet
{
    public class DashboardManager
    {
            ...
            PictureBox ms_fuel)
        {
            ...
            serialManager.OnDataReceived += async (data) => 
            {
                if (data == BOOT_START_MESSAGE)
                {
                    isInitializing = true;
                    // Reset all states during boot sequence
                    HandleVehicleChanged(omsiManager.CurrentVehicle);
                    return;
                }
                else if (data == BOOT_FINISH_MESSAGE)
                {
                    // Wait 1 second after receiving boot finished message
