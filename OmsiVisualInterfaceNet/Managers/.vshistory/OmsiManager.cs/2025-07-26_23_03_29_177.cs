...
namespace OmsiVisualInterfaceNet
{
    public class OmsiManager : IDisposable
    {
        ...
        public bool GetDoorState(int door)
        {
            ...
            {
                if (Convert.ToBoolean(CurrentVehicle.GetVariable($"door_23")) || Convert.ToBoolean(CurrentVehicle.GetVariable($"door_23")))
                    return true;
            }
            else if (door == 2)
            {
                if (Convert.ToBoolean(CurrentVehicle.GetVariable($"door_45")) || Convert.ToBoolean(CurrentVehicle.GetVariable($"door_45")))
                    return true;
