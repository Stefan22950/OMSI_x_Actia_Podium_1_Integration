...
namespace OmsiVisualInterfaceNet
{
    public class DashboardManager
    {
        ...
        {
            // Convert speed to appropriate format and send immediately
            int speedKmh = (int)Math.Round(speed * 3.6);
            serialManager.WriteHighPriority($"SPEED:{speedKmh}");
        }

        private void UpdateTachometer(double rpm)
        {
            // Send RPM updates with high priority
            int rpmValue = (int)Math.Round(rpm);
            serialManager.WriteHighPriority($"RPM:{rpmValue}");
        }

        public void Update()
        {
            // Update speed and RPM first for minimal latency
            UpdateSpeedometer(omsiManager.GetSpeed());
            UpdateTachometer(omsiManager.GetRPM());

            // Then handle other less time-critical updates
            UpdatePowerState(omsiManager.GetVariable("elec") > 0);
            UpdateBlinkerState();
            UpdateHazardLights();
            UpdateGearIndicator();
