...
namespace OmsiVisualInterfaceNet
{
    public class OmsiManager : IDisposable
    {
        ...
        public void SetLightSwitch(int pos)
        {
            ...
            //CurrentVehicle?.SetVariable("actia_schluessel_rot ", pos);
            //var var2 = CurrentVehicle?.GetVariable("cp_schluessel_rot");

            if (pos == 0)
            {
                CurrentVehicle?.SetTrigger("kw_standlicht_toggle", false);
                CurrentVehicle?.SetTrigger("kw_scheinwerfer_toggle", false);
            }
            else if (pos == 1)
            {
