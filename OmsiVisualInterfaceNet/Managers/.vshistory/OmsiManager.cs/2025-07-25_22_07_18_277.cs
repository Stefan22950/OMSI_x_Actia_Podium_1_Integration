...
namespace OmsiVisualInterfaceNet
{
    public class OmsiManager : IDisposable
    {
        ...
        public void SetDoorButton(int door, bool pressed)
        {
            ...
            if (door == 0)
            {
                CurrentVehicle?.SetTrigger($"bus_doorfront0", pressed ? true : false);
                CurrentVehicle?.SetVariable($"doorTarget_0", pressed ? 1 : 0);
                CurrentVehicle?.SetTrigger($"bus_doorfront1", pressed ? true : false);
                CurrentVehicle?.SetVariable($"doorTarget_1", pressed ? 1 : 0);
            }
            else if (door == 1)
