...
namespace OmsiVisualInterfaceNet
{
    public class DashboardManager
    {
        ...
        private void UpdateAirPressure()
        {
            ...
            double ai1Level = Convert.ToDouble(omsiManager.CurrentVehicle.GetVariable("bremse_p_Tank01"));
            ...
            ai1Level = Math.Clamp(ai1Level, 0, fuelcapacity);
            double ratio = ai1Level / fuelcapacity; // 0.0 to 1.0
            int widthfuel = (int)(minWidth + ratio * (maxWidth - minWidth));
            fuelPanel.Width = widthfuel;

            air2Level = Math.Clamp(air2Level, 0, 1);
            int widthadblue = (int)(minWidth + air2Level * (maxWidth - minWidth));
            adbluePanel.Width = widthadblue;

            // Update low fuel warning light only if state changes
            bool lowair = ai1Level < 0.2;
