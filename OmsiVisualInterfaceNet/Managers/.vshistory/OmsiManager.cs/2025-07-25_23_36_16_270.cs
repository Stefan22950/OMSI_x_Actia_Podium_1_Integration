...
namespace OmsiVisualInterfaceNet
{
    public class OmsiManager : IDisposable
    {
        ...
        public OmsiManager(SerialManager SerialManager)
        {
        ...
        public void Update()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                if (doorOpen != lastDoorStates[i])
                {
                    ...
                    {
                        // Check if lights are on to determine if we should dim or turn off completely
                        bool headlightsOn = GetLightSwitch() > 0;
                        if (headlightsOn)
        ...
        public void SetDoorButton(int door, bool pressed)
        {
            ...
            {
                case 0: // Front doors
                    triggerVar = doorOpen ? "bus_doorfront0" : "bus_doorfront0_off";
                    targetVar = "doorTarget_0";
                    break;
                case 1: // Middle doors
                    triggerVar = doorOpen ? "bus_doorfront23" : "bus_doorfront23_off";
                    targetVar = "doorTarget_23";
                    break;
                case 2: // Rear doors
                    triggerVar = doorOpen ? "bus_doorfront45" : "bus_doorfront45_off";
                    targetVar = "doorTarget_45";
                    break;
                default:
                    return;
            }

            // Set door trigger and target
            CurrentVehicle?.SetTrigger(triggerVar, !doorOpen);
            CurrentVehicle?.SetVariable(targetVar, doorOpen ? 0 : 1);

            // For front doors, we need to handle both doors
            if (door == 0)
            {
