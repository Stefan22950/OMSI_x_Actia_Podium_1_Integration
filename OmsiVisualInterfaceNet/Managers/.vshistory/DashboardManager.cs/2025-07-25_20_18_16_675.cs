...
namespace OmsiVisualInterfaceNet
{
    public class DashboardManager
    {
        ...
        private void UpdateClusterLights()
        {
            ...
            bool nightLight = omsiManager.GetLowBeamState() || omsiManager.GetParkingLightState();
            serialManager.WriteLine($"NIGHT_LIGHT:{(nightLight ? "ON" : "OFF")}");
            ...
            serialManager.WriteLine($"LOW_BEAM_LIGHT:{(lowBeam ? "ON" : "OFF")}");
        }
        ...
        public void HandleSerialInput(string input)
        {
            ...
            switch (input)
            {
                ...
                // Door controls
                case "DOOR_1_PRESSED":
                    omsiManager.SetDoorState(0,true);
                    break;
                case "DOOR_2_PRESSED":
                    omsiManager.SetDoorState(1, true); ;
                    break;
                case "DOOR_3_PRESSED":
                    omsiManager.SetDoorState(2, true);
                    break;
                case "AUTO_DOOR_ON":
