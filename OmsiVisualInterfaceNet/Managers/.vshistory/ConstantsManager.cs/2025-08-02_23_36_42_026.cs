...
namespace OmsiVisualInterfaceNet.Managers
{
    class ConstantsManager
    {
        public void Main()
        {
            string omsiPath = @"C:\OMSI2"; // Your OMSI base path
            ...
            // 3. Search for the constant in all files in script folder
            double? value = FindConstantValue(scriptFolder, "engine_tank_capacity");
            if (value.HasValue)
                Console.WriteLine("engine_tank_capacity = " + value.Value);
            else
        ...
        public double? FindConstantValue(string scriptFolder, string constantName)
        {
