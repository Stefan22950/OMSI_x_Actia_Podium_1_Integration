...
namespace OmsiVisualInterfaceNet
{
    public class OmsiManager : IDisposable
    {
        ...
        public OmsiManager(SerialManager SerialManager,DashboardManager DashboardManager, bool EnableLogging)
        {
        ...
        public void Update()
        {
            ...
            for (int i = 0; i < 3; i++)
            {
                ...
                if (doorOpen != lastDoorStates[i])
                {
                    ...
                    {
                        bool headlightsOn = dashboardManager.lastNightLightState > 0;
                        if (headlightsOn)
