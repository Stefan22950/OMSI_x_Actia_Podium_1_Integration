...
namespace OmsiVisualInterfaceNet
{
    public class OmsiManager : IDisposable
    {
        ...
        private LightState lastLightState;
        ...
        {
            if(currentLightState != LightState.LIGHTS_OFF)
            {
                return true;
            }
            return false;
        }
        public bool GetLowBeamState()
        {
            if (currentLightState == LightState.LOW_BEAM)
            {
                return true;
            }
            return false;
        ...
        public float GetLightSwitch() => CurrentVehicle == null ? 0 : Convert.ToInt32(CurrentVehicle?.GetVariable("cp_schluessel_rot"));
            ...
            /*if (pos == 0)
             {
                ...
                currentLightState = LightState.LIGHTS_OFF;
             }
             else if (pos == 1)
             {
                 CurrentVehicle?.SetTrigger("kw_standlicht_toggle", true);
                currentLightState = LightState.PARKING_LIGHTS;
             }
             else if (pos == 2)
             {
                 CurrentVehicle?.SetTrigger("kw_scheinwerfer_toggle", true);
                currentLightState = LightState.LOW_BEAM;
             }*/
