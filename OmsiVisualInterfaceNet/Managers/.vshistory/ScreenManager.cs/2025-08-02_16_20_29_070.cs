...
namespace OmsiVisualInterfaceNet.Managers
{
    public class ScreenManager
    {
        ...
        private System.Windows.Forms.Timer startupTimerTicker;
        ...
        private void StartupTimerTicker_Tick(object? sender, EventArgs e)
        {
            ...
            if (startupTimerSeconds == 1.0)
            {
                HideAllScreens();
            }
            else if (startupTimerSeconds > 1.0 && startupTimerSeconds < 3.0)
            {
                ShowScreen(LogoScreen);
                SetAllIconsVisible(false);
                return;
            }
            else if (startupTimerSeconds > 3.0 && startupTimerSeconds < 5.0)
            {
                ShowScreen(PressureScreen);
                SetAllIconsVisible(false);
                return;
            }
            else if (startupTimerSeconds > 5.0 && startupTimerSeconds < 7.0)
            {
                ShowScreen(FuelScreen);
                SetAllIconsVisible(false);
                return;
            }
            else if (startupTimerSeconds > 7.0 && startupTimerSeconds < 10.0)
            {
                ShowScreen(CoolantTemperatureScreen);
                SetAllIconsVisible(false);
                return;
            }
            else if (startupTimerSeconds > 10.0)
            {
