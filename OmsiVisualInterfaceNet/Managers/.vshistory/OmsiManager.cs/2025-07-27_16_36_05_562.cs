...
namespace OmsiVisualInterfaceNet
{
    public class OmsiManager : IDisposable
    {
        ...
        public void SetLightSwitch(float pos)
        {
            ...
            if (pos < 1)
            {
                if (lastHighBeamState)
                    SetHighBeam(false); // Actually turns off high beam, but intent remains
            }
        ...
        {
            highBeamIntent = on; // Always update intent

            // Only actually set high beam if low beam is active
